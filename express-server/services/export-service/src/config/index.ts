...
export const config = {
  app: {
    ...
    port: optionalEnvInt('PORT', 3007),
    serviceName: optionalEnv('SERVICE_NAME', 'export-service'),
    isProduction: optionalEnv('NODE_ENV', 'development') === 'production',
    isTest: optionalEnv('NODE_ENV', 'development') === 'test',
  },
  mongodb: {
    uri: optionalEnv('MONGODB_URI', 'mongodb://localhost:27017/devdocs_export'),
  ...
  rabbitmq: {
    ...
    queue: optionalEnv('RABBITMQ_QUEUE', 'export-service-queue'),
  },
  logging: {
  ...
  services: {
    documentServiceUrl: optionalEnv('DOCUMENT_SERVICE_URL', 'http://localhost:3004'),
    fileServiceUrl: optionalEnv('FILE_SERVICE_URL', 'http://localhost:3008'),
  },
  export: {
    storagePath: optionalEnv('EXPORT_STORAGE_PATH', '/tmp/exports'),
    maxSizeMb: optionalEnvInt('MAX_EXPORT_SIZE_MB', 50),
    expiryHours: optionalEnvInt('EXPORT_EXPIRY_HOURS', 24),
  },
} as const;
