...
export const config = {
  app: {
    ...
    port: optionalEnvInt('PORT', 3010),
    serviceName: optionalEnv('SERVICE_NAME', 'audit-service'),
    isProduction: optionalEnv('NODE_ENV', 'development') === 'production',
    isDevelopment: optionalEnv('NODE_ENV', 'development') === 'development',
    isTest: optionalEnv('NODE_ENV', 'development') === 'test',
  },
  mongodb: {
    uri: optionalEnv('MONGODB_URI', 'mongodb://localhost:27017/devdocs_audit'),
  ...
  rabbitmq: {
    ...
    queue: optionalEnv('RABBITMQ_QUEUE', 'audit-service-queue'),
  },
  rateLimit: {
  ...
  retention: {
    days: optionalEnvInt('AUDIT_RETENTION_DAYS', 365),
  },
} as const;
